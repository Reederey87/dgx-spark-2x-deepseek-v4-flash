#!/usr/bin/env bash
# Build the #51318 (C128A adaptive-packing revert) derivative, stacked on -ixfix;
...
BASE="${BASE:-vllm-dspark-runtime:v0261-main-c8r-tbfix-ixfix}"
TAG="${TAG:-vllm-dspark-runtime:v0261-main-c8r-tbfix-ixfix-c128arev}"
REMOTE_HOME="$(ssh "$HEAD_SSH" 'printf %s "$HOME"')"
REMOTE="${REMOTE:-$REMOTE_HOME/build/vllm-0261-main-c128arev}"
TARBALL="vllm-c128arev.tar.zst"
DISTRIBUTE=0
...
ssh "$HEAD_SSH" "docker run --rm --entrypoint grep '$TAG' -q \
  'global_decode_buffer.stride(0)' '$pkg_dir/models/deepseek_v4/sparse_mla.py' && echo 'c128arev marker OK'"
