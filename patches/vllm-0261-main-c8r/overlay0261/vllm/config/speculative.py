            ...
            if self.model is not None:
                ...
                if self.method == "dspark":
                    ...
                    if (
                        ...
                        # LOCAL OVERLAY (2xSPARK-CLUSTER, experiment E17; tracks
                        # upstream issue vllm-project/vllm#50012): warn-only.
                        # The garble claim is refuted on the GB10 fork stack:
                        # n=3 < block_size 5 is garble-clean (needle HITs to
                        # 944K ctx, finish=stop long generations, acceptance
                        # ~0.43 bench / 0.776 battery), neither the fork's nor
                        # upstream's speculator has any block-size-dependent
                        # code path, and rejection sampling makes draft length
                        # a throughput-only variable. Complying (n=5) costs
                        # -12..-17% batched throughput (A/B'd three times).
                        logger.warning(
                            "DSpark: num_speculative_tokens (%d) < "
                            "dspark_block_size (%d); upstream guard downgraded "
                            "to warning by local overlay (measured healthy on "
                            "this stack — see #50012). Proceeding with n=%d.",
                            self.num_speculative_tokens,
                            dspark_block_size,
                            self.num_speculative_tokens,
                        )
