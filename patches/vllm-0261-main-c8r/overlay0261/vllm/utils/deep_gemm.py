...
# LOCAL OVERLAY (2xSPARK-CLUSTER, cand8): SM120/SM121 SF-layout transform port.
# Main's vendored DeepGEMM pin moved to the vllm-project fork (e21c821, via #51003),
# whose csrc/apis/layout.hpp DROPPED the arch_major==12 branches from
# transform_sf_into_required_layout (now SM100-only) → GB10 boots die at weight-load:
# "RuntimeError: Assertion error (layout.hpp): Unknown SF transformation" in
# deepgemm_post_process_fp8_weight_block. The prod 0.26.0 image pins deepseek-ai
# a6b593d, which handles sm12x via get_mn_major_tma_aligned_packed_ue8m0_tensor.
# Below is a 1:1 torch port of that pin's reference implementation
# (csrc/jit_kernels/impls/smxx_layout.hpp:149-172,
# get_mn_major_tma_aligned_packed_ue8m0_tensor_torch), reached only on sm12x for the
# FP32 case — the exact inputs the new pin asserts on. Runs at weight-load time only;
# zero runtime-perf effect. Bit-exactness vs the a6b593d C++ binding is verified
# against the cand7 prod image before any lane boot (see patches/vllm-0261-main/).
# NOTE: the raw-DG-module call site in models/deepseek_v4/nvidia/model.py (MegaMoE
# finalize_weights) is NOT covered — that backend is not selected on GB10.
def _dg_sm12x_packed_ue8m0_layout(sf: torch.Tensor) -> torch.Tensor:
    sf = sf.contiguous()
    sf_reshaped = sf.unsqueeze(0) if sf.dim() == 2 else sf

    # Convert into UE8M0 uint8 (exponent bits of the fp32 scale).
    ue8m0_tensor = sf_reshaped.view(torch.int32).bitwise_right_shift(23).to(torch.uint8)

    # Pad to TMA alignment: get_tma_aligned_size(mn, 4) = align(mn, 16/4); align(k, 4).
    num_groups, mn, k = sf_reshaped.shape
    aligned_mn = (mn + 3) // 4 * 4
    aligned_k = (k + 3) // 4 * 4

    padded = torch.zeros(
        (num_groups, aligned_mn, aligned_k), device=sf.device, dtype=torch.uint8
    )
    padded[:, :mn, :k].copy_(ue8m0_tensor)
    padded = padded.view(-1).view(torch.int32).view(num_groups, aligned_mn, aligned_k // 4)

    # Transpose to MN-major via strided copy, then slice off the mn padding.
    out = torch.empty_strided(
        (num_groups, aligned_mn, aligned_k // 4),
        (aligned_mn * (aligned_k // 4), 1, aligned_mn),
        device=sf.device,
        dtype=torch.int32,
    )
    out = out.copy_(padded)[:, :mn]
    return out.squeeze(0) if sf.dim() == 2 else out


def _dg_sm12x_sf_transform() -> bool:
...
def transform_sf_into_required_layout(*args, **kwargs):
    ...
    if _transform_sf_into_required_layout_impl is None:
        return _missing(*args, **kwargs)
    # LOCAL OVERLAY (above): intercept the FP32 SF case on sm12x — the vendored DG
    # pin asserts there ("Unknown SF transformation"). recipe is (1, gran_mn, gran_k);
    # the gran_mn != 1 broadcast mirrors the old pin's index_select branch.
    sf = kwargs.get("sf", args[0] if args else None)
    if (
        isinstance(sf, torch.Tensor)
        and sf.dtype == torch.float32
        and _dg_sm12x_sf_transform()
    ):
        recipe = kwargs.get("recipe", args[3] if len(args) > 3 else None)
        if recipe is not None and len(recipe) == 3 and recipe[2] in (32, 128):
            gran_mn = recipe[1]
            if gran_mn != 1:
                mn = kwargs.get("mn", args[1] if len(args) > 1 else None)
                sf = sf.index_select(
                    -2,
                    torch.arange(mn, device=sf.device).floor_divide_(gran_mn),
                )
            return _dg_sm12x_packed_ue8m0_layout(sf)
