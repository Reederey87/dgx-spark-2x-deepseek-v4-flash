    ...
    def _forward_prefill(
        ...
        for chunk_idx in range(num_chunks):
            ...
            if q_chunk.shape[0] == 0:
                # 0.26 can emit zero-token prefill chunks in some batch
                # compositions (async scheduling + spec decode): the flattened
                # query array contains a zero-length span for a counted prefill
                # request, and an empty launch crashes flashinfer's segment
                # normalize (reshape of 0 elements into [0, -1]). Zero tokens =
                # zero work — skip. (Upstream precedent: #48957 skips empty
                # c128 kernel launches.) Local overlay, goal P2 fix-iter-2.
                logger.warning_once(
                    "Skipping zero-token sparse prefill chunk "
                    f"(chunk {chunk_idx}, requests {chunk_start}:{chunk_end})"
                )
                continue
