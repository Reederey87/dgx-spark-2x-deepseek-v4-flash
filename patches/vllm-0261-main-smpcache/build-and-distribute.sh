#!/usr/bin/env bash
# Build the #52329 (MRV2 cached logits-processing state) derivative, stacked on
# -c128arev; optionally distribute it. This layer REPLACES sampler.py wholesale and
# thereby retires the tbfix patch (upstream's cached predicate carries the
# thinking-budget term). Mirrors patches/vllm-0261-main-tbfix/build-and-distribute.sh.
set -euo pipefail
...
BASE="${BASE:-vllm-dspark-runtime:v0261-main-c8r-tbfix-ixfix-c128arev}"
TAG="${TAG:-vllm-dspark-runtime:v0261-main-c8r-tbfix-ixfix-c128arev-smpcache}"
REMOTE_HOME="$(ssh "$HEAD_SSH" 'printf %s "$HOME"')"
REMOTE="${REMOTE:-$REMOTE_HOME/build/vllm-0261-main-smpcache}"
TARBALL="vllm-smpcache.tar.zst"
DISTRIBUTE=0
...
ssh "$HEAD_SSH" "docker run --rm --entrypoint grep '$TAG' -q \
  needs_logits_processing '$pkg_dir/v1/worker/gpu/sample/sampler.py' && echo 'smpcache marker OK'"
