#!/usr/bin/env bash
# Build the small c8r thinking-budget derivative; optionally distribute it.
set -euo pipefail
...
BASE="${BASE:-${DSPARK_VLLM_ROLLBACK_IMAGE:-vllm-dspark-runtime:v0261-main-c8r}}"
TAG="${TAG:-${DSPARK_VLLM_IMAGE:-vllm-dspark-runtime:v0261-main-c8r-tbfix}}"
REMOTE_HOME="$(ssh "$HEAD_SSH" 'printf %s "$HOME"')"
REMOTE="${REMOTE:-$REMOTE_HOME/build/vllm-0261-main-tbfix}"
TARBALL="vllm-c8r-tbfix.tar.zst"
DISTRIBUTE=0
...
ssh "$HEAD_SSH" "docker run --rm --entrypoint python3 '$TAG' -c \
  'from vllm.v1.worker.gpu.sample.sampler import Sampler; import inspect; s=inspect.getsource(Sampler._requires_logits_processing); assert \"use_thinking_budget\" in s; print(\"tbfix marker OK\")'"
